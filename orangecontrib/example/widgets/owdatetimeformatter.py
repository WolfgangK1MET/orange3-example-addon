...
class DatetimeFormatter(OWWidget):
    ...
    def __init__(self):
        ...
        self.__cb_include_milliseconds = gui.checkBox(self.__options_box, self, "include_milliseconds",
                                                      "Include Milliseconds", callback=self.reload_data)
    ...
    def _set_output_data(self):
        ...
        if detected_format == DatetimeFormat.ISO_DATETIME:
            self.__cb_include_milliseconds.setDisabled(True)
            try:
                time_strings = TableUtility.get_datetime_strings_from_single_column(self.__input_dataset, "ISOZEIT",
                                                                                    "%d.%m.%Y %H:%M:%S")
            except:
                self.Error.unsupported_date_format(shown=True)
                return handled

            domain_attributes, domain_metas = TableUtility.domain_extract_columns(self.__input_dataset.domain,
                                                                                  "ISOZEIT")

        elif detected_format == DatetimeFormat.DE_DATETIME_SEPARATE:
            try:
                time_strings = TableUtility.get_datetime_strings_from_columns(self.__input_dataset, "Datum", "Uhrzeit",
                                                                              "%d.%m.%Y %H:%M:%S,%f",
...
class TableUtility:
    ...
    def get_datetime_strings_from_columns(dataset, date_col_name, time_col_name, formating, include_milliseconds):
        ...
        for row in dataset:
            strings.append(
                TableUtility.datetime_string_to_iso8601(f'{row[date_col_name]}' + " " + f'{row[time_col_name]}',
                                                        formating, include_milliseconds))
    ...
    def generate_new_table(dataset, domain_attributes, domain_metas):
        new_domain = Domain(attributes=tuple(domain_attributes), metas=domain_metas,
                            class_vars=dataset.domain.class_vars)
